...
# hrow = 1 .. maxrow
def is_horizontal_smudged(arr, hrow):
    ...
    for r in range(0, numrows):
        for c in range(0, maxcol):
            numdiffs += 0 if arr[hrow-r-1][c] == arr[hrow+r+1-1][c] else 1
    return numdiffs == 1

# vcol = 1 .. maxcol
def is_vertical_smudged(arr, vcol):
    maxrow, maxcol = arr.shape
    numleftcols = vcol
    ...
    for c in range(0, numcols):
        for r in range(0, maxrow):
            numdiffs += 0 if arr[r][vcol-c-1] == arr[r][vcol+c+1-1] else 1
    return numdiffs == 1
...
def score_for_array(arr):
    ...
    for col in range(1, maxcol):
        if is_vertical_smudged(arr, col):
            s += col
    for row in range(1, maxrow):
        if is_horizontal_smudged(arr, row):
            s += 100 * row
    return s
